...
import org.onap.ccsdk.features.sdnr.wt.dataprovider.database.mariadb.query.SqlQuery;
import org.onap.ccsdk.features.sdnr.wt.dataprovider.database.mariadb.query.UpdateQuery;
import org.onap.ccsdk.features.sdnr.wt.dataprovider.database.mariadb.query.UpsertQuery;
...
public class MariaDBReaderWriter<T extends DataObject> {

    private static final Logger LOG = LoggerFactory.getLogger(MariaDBReaderWriter.class);

    protected final Entity entity;
    private final Class<T> clazz;
    protected final MariaDBClient dbService;
    private final String dbName;
    protected final String controllerId;
    protected final String tableName;

    public MariaDBReaderWriter(MariaDBClient dbService, Entity e, String dbSuffix, Class<T> clazz, String dbName,
            String controllerId) {
    ...
    public long count(List<Filter> list, String controllerId) throws SQLException {
        ...
        Optional<Filter> cFilter =
                list.stream().filter(e -> MariaDBMapper.ODLID_DBCOL.equals(e.getProperty())).findFirst();
        if (!cFilter.isEmpty()) {
            list.remove(cFilter.get());
        }
        if (controllerId != null) {
            list.add(new FilterBuilder().setProperty(MariaDBMapper.ODLID_DBCOL).setFiltervalue(this.controllerId)
                    .build());
        }
    ...
    public QueryResult<T> getData(EntityInput input) {
        ...
        try {
            ...
            List<T> mappedData = MariaDBMapper.read(data, clazz);
    ...
    public MariaDBReaderWriter<T> setWriteInterface(Class<? extends DataObject> writeInterfaceClazz) {
    ...
    public <S extends DataObject> List<S> readAll(Class<S> clazz) {
        ...
        try {
            ResultSet data = this.dbService.read(query.toSql());
            List<S> mappedData = MariaDBMapper.read(data, clazz);
            return mappedData;
        } catch (SQLException | IllegalAccessException | IllegalArgumentException | InvocationTargetException
                | InstantiationException | SecurityException | NoSuchMethodException | JsonProcessingException e) {
            LOG.warn("problem reading data {}: ", this.entity, e);
    ...
    public List<String> readAll(String key) {
        ...
        try {
            ResultSet data = this.dbService.read(query.toSql());
            List<String> mappedData = MariaDBMapper.read(data, String.class, key);
            return mappedData;
        } catch (SQLException | IllegalAccessException | IllegalArgumentException | InvocationTargetException
                | InstantiationException | SecurityException | NoSuchMethodException | JsonProcessingException e) {
            LOG.warn("problem reading data {}: ", this.entity, e);
