...
import org.onap.ccsdk.features.sdnr.wt.dataprovider.database.mariadb.query.filters.DBKeyValuePair;
...
public class UpdateQuery<T extends DataObject> implements SqlQuery {
    ...
    private String toSqlWithError() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException,
        ...
        List<Method> methods = MariaDBMapper.getFilteredMethods(cls, true);
        Method m;
        for (int i = 0; i < methods.size(); i++) {
            m = methods.get(i);
            m.setAccessible(true);
            value = m.invoke(this.object);
            col = MariaDBMapper.getColumnName(m);
            if (col.equals("id")) {
                if (this.id == null) {
            ...
            DBKeyValuePair<String> kvp = MariaDBMapper.getEscapedKeyValue(m, col, value);
            args.add(String.format("%s=%s", kvp.getKey(), kvp.getValue()));
        }
        sb.append(String.join(",", args));
        sb.append(String.format(" WHERE `id`='%s'", this.id));
        if (this.controllerId != null) {
            sb.append(String.format(" AND `%s`='%s'", MariaDBMapper.ODLID_DBCOL, this.controllerId));
        }
