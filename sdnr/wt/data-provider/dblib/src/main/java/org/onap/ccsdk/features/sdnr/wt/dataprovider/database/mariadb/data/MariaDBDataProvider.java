 ...
 *
 */
package org.onap.ccsdk.features.sdnr.wt.dataprovider.database.mariadb.data;
...
import org.onap.ccsdk.features.sdnr.wt.dataprovider.database.mariadb.data.entity.HtDatabaseEventsService;
import org.onap.ccsdk.features.sdnr.wt.dataprovider.database.mariadb.data.entity.HtDatabaseMaintenanceService;
...
import org.onap.ccsdk.features.sdnr.wt.dataprovider.database.mariadb.database.MariaDBStatusReader;
...
public class MariaDBDataProvider extends HtDatabaseEventsService implements DatabaseDataProvider {

    private static final Logger LOG = LoggerFactory.getLogger(MariaDBDataProvider.class);
    ...
    private final MariaDBReaderWriter<org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.data.provider.rev201110.read.mediator.server.list.output.Data> mediatorserverRW;
    private final MariaDBReaderWriter<org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.data.provider.rev201110.read.maintenance.list.output.Data> maintenanceRW;
    private final MariaDBStatusReader readStatus;

    public MariaDBReaderWriter<org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.data.provider.rev201110.read.maintenance.list.output.Data> getMaintenanceReaderWriter() {
        return this.maintenanceRW;
    }

    public MariaDBReaderWriter<Data> getMediatorServerReaderWriter() {
        return this.mediatorserverRW;
    }

    public MariaDBDataProvider(MariaDBConfig config) {
        this(config, true);
    }
    public MariaDBDataProvider(MariaDBConfig config, boolean initControllerId) {
        super(config);

        this.mediatorserverRW = new MariaDBReaderWriter<>(this.dbClient, Entity.MediatorServer, config.getDbSuffix(),
        ...
        this.maintenanceRW = new MariaDBReaderWriter<>(this.dbClient, Entity.Maintenancemode, config.getDbSuffix(),
        ...
        this.readStatus = new MariaDBStatusReader(this.dbClient, this.controllerId);

        this.dbMediatorServerService = new HtDatabaseMediatorserver() {

            @Override
            public List<MediatorServerEntity> getAll() {
                return MariaDBDataProvider.this.mediatorserverRW.readAll(MediatorServerEntity.class);
            }
        };
    ...
    public MariaDBClient getDBService() {
