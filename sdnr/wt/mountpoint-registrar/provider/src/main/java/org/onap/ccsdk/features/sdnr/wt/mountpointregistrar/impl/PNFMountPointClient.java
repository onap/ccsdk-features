...
public class PNFMountPointClient extends BaseHTTPClient {
    ...
    private static String updatePayload(String pnfName, String ipv4Address, String username, String password,
            String portNo) {
        return SSH_PAYLOAD.replace("@device-name@", pnfName).replace("@device-ip@", ipv4Address)
                .replace("@device-port@", portNo).replace("@username@", username).replace("@password@", password);
    }

    private static String updateTLSPayload(String pnfName, String ipv4Address, String username, String keyId,
            String portNo) {
        return TLS_PAYLOAD.replace("@device-name@", pnfName).replace("@device-ip@", ipv4Address)
                .replace("@username@", username).replace("@key-id@", keyId).replace("@device-port@", portNo);
    }
