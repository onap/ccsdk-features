...
public class DMaaPCMVESMsgConsumer extends DMaaPVESMsgConsumerImpl {
    ...
    public void processMsg(String msg) throws InvalidMessageException, JsonProcessingException {
        ...
        try {
            ...
            if (notificationType.equalsIgnoreCase("notifyMOIChanges")) {
                ...
                sendCMNotification(processMoiCreationDeletion(rootNode));
                ...
                sendCMNotification(processMoiAttributeValueChanges(rootNode));
            } else {
    ...
    private void processMoiChanges(JsonNode rootNode) {
        Iterator<JsonNode> nodes = rootNode
            ...
            .elements();
        while (nodes.hasNext()) {
            sendCMNotification(processFieldFromMoiChangesArray(rootNode, nodes));
        }
    }

    public Map<String, String> processFieldFromMoiChangesArray(JsonNode rootNode, Iterator<JsonNode> nodes) {
    ...
    public Map<String, String> processMoiAttributeValueChanges(JsonNode rootNode) {
        return CMNotificationClient.createCMNotificationPayloadMap(
            CMNotification.builder()
                .withCMBasicHeaderFieldsNotification(
                    prepareCMCommonHeaderFields(rootNode))
                .withCMSourceIndicator(rootNode.at("/event/stndDefinedFields/data/sourceIndicator").textValue())
                .withCMValue(rootNode.at("/event/stndDefinedFields/data/attributeListValueChanges").toString()
                    .replace("\"", ""))
                .build());
    }

    public Map<String, String> processMoiCreationDeletion(JsonNode rootNode) {
        return CMNotificationClient.createCMNotificationPayloadMap(
            CMNotification.builder()
                .withCMBasicHeaderFieldsNotification(
                    prepareCMCommonHeaderFields(rootNode))
                .withCMSourceIndicator(rootNode.at("/event/stndDefinedFields/data/sourceIndicator").textValue())
                .withCMValue(rootNode.at("/event/stndDefinedFields/data/attributeList").toString()
                    .replace("\"", ""))
                .build());
    }

    private boolean sendCMNotification(Map<String, String> payloadMapMessage) {
        ...
        return cmClient.sendNotification(message);
    }
