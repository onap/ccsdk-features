...
const resolveViewDescription = (vPath: string, view: ViewSpecification, viewData: any, displayAsList: boolean = false, key?: string): UpdatViewDescription =>{
...
export const updateViewActionAsyncCreator = (vPath: string) => async (dispatch: Dispatch, getState: () => IApplicationStoreState) => {
  ...
  try {
    for (let ind = 0; ind < pathParts.length; ++ind) {
      ...
      if (viewElement.isList && !key) {
        if (pathParts.length - 1 > ind) {
          ...
          return dispatch(resolveViewDescription(vPath, viewSpecification, data, false, isViewElementList(viewElement!) && viewElement.key || undefined));
        }
    ...
    if (viewSpecification && viewSpecification.id !== "0") {
      ...
      if (!restResult.data) {
        ...
        if (extractList && restResult.status === 404) {
          ...
          return dispatch(resolveViewDescription(vPath, viewSpecification, [], extractList, viewElement.key));
        }
    ...
    return dispatch(resolveViewDescription(vPath, viewSpecification, data, extractList, isViewElementList(viewElement!) && viewElement.key || undefined));
...
export const updateDataActionAsyncCreator = (vPath: string, data: any) => async (dispatch: Dispatch, getState: () => IApplicationStoreState) => {
  ...
  try {
    ...
    : dispatch(resolveViewDescription(vPath, viewSpecification, data, embedList, isViewElementList(viewElement!) && viewElement.key || undefined));
  } catch (error) {
    history.back();
