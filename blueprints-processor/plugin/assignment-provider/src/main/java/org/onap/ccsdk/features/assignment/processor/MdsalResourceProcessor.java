...
public class MdsalResourceProcessor implements ComponentNode {
  ...
  private void processResourceAssignmnet(SvcLogicContext ctx, Map<String, Object> componentContext,
    ...
    try {
      ...
      if (value != null) {
        logger.info("mdsal source template key ({}) found from input and value is ({})", resourceAssignment.getName(),
            value);
      ...
      logger.info("mdsal dictionary information : urlpath ({}), path({}), inputKeyMapping ({}), outputKeyMapping ({})",
      ...
      if (StringUtils.isNotBlank(restResponse)) {
        ...
        logger.warn("Coudn't get proper mdsal Response content ({}) for Resource Name ({}) for URI ({})", restResponse,
            resourceAssignment.getDictionaryName(), urlPath);
      }
      ...
      throw new SvcLogicException(String.format("Failed in assignments for (%s) with (%s)", resourceAssignment, e), e);
    }
  }
  ...
  private String fetchResourceFromMDSAL(String urlPath) {
    ...
    try {
      response =
          configRestAdaptorService.getResource(ConfigRestAdaptorConstants.SELECTOR_RESTCONF, urlPath, String.class);
    } catch (Exception e) {
  ...
  private void validate(ResourceAssignment resourceAssignment) throws SvcLogicException {
    ...
    if (StringUtils.isBlank(resourceDefinition.getProperty().getType())) {
      throw new SvcLogicException(String.format(
          String.format("Failed to get dictionary (%s) data type info.", resourceAssignment.getDictionaryName())));
    }

    if (resourceDefinition.getSources() == null || resourceDefinition.getSources().get("mdsal") == null) {
      throw new SvcLogicException(String.format("missing resource dictionary mdsal source definition for name (%s)  ",
          resourceAssignment.getDictionaryName()));
    }
    ...
    if (StringUtils.isBlank(sourceMdsal.getProperties().getUrlPath())) {
      throw new SvcLogicException(
          String.format("Failed to get request URL Path for dictionary (%s)", resourceAssignment.getDictionaryName()));
    }

    if (StringUtils.isBlank(sourceMdsal.getProperties().getPath())) {
      throw new SvcLogicException(
          String.format("Failed to get request Path for dictionary (%s)", resourceAssignment.getDictionaryName()));
    }
  }
  ...
  private void processMdsalResults(SvcLogicContext ctx, Map<String, Object> componentContext,
    ...
    if (responseNode != null) {
      ConfigAssignmentUtils.populateValueForOutputMapping(ctx, componentContext, resourceAssignment, outputKeyMapping,
          responseNode);
    }
  }
