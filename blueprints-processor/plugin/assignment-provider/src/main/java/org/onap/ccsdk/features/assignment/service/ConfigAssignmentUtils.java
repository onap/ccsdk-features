...
public class ConfigAssignmentUtils {
  ...
  public static synchronized void populateValueForOutputMapping(SvcLogicContext ctx,
      Map<String, Object> componentContext, ResourceAssignment resourceAssignment, Map<String, String> outputKeyMapping,
      JsonNode responseNode) throws ConfigModelException, SvcLogicException {
    ...
    if (ValidTypes.getPrimitivePropertType().contains(type)) {
          ...
          for (JsonNode responseSingleJsonNode : responseArrayNode) {
            if (responseSingleJsonNode != null) {
              ...
              for (Map.Entry<String, String> mapping : outputKeyMapping.entrySet()) {
                ...
                logger.info("For List Type Resource: key ({}), value ({}), type  ({})", mapping.getKey(),
      ...
      if (MapUtils.isNotEmpty(outputKeyMapping)) {
        ...
        for (Map.Entry<String, String> mapping : outputKeyMapping.entrySet()) {
          ...
          String propertyTypeForDataType = ConfigAssignmentUtils.getPropertyType(ctx, entrySchema, mapping.getKey());
          logger.info("For Complex Type Resource: key ({}), value ({}), type  ({})", mapping.getKey(), responseKeyValue,
              propertyTypeForDataType);
          JsonUtils.populateJsonNodeValues(mapping.getValue(), responseKeyValue, propertyTypeForDataType, objectNode);
        }
      } else {
        ...
        if (resourceAssignmentData != null) {
          ...
          if (StringUtils.isNotBlank(resourceAssignmentData.getResourceValue())) {
            if (ValidTypes.getPrimitivePropertType().contains(resourceAssignmentData.getDataType())) {
              ...
              JsonNode valueNode = TransformationUtils.getJsonNodeForString(resourceAssignmentData.getResourceValue());
