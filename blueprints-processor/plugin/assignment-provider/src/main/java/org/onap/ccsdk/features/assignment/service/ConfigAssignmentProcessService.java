...
public class ConfigAssignmentProcessService {
        ...
        for (String templateName : templateNames) {
          ...
          if (resourceAssignments != null) {
            ...
            try {
              ...
              processResourceAssignments(resourceAssignmentData, svcLogicContext, componentContext, templateName,
                  resourceAssignments, dictionaries);

              logger.info("decrypting config data for templateName {}", templateName);
              templateData = ResourceAssignmentUtils.generateResourceDataForAssignments(resourceAssignments);
            } finally {
  ...
  private void processResourceAssignments(ResourceAssignmentData resourceAssignmentData, SvcLogicContext ctx,
    ...
    ResourceAssignmentProcessor resourceAssignmentProcessor = new ResourceAssignmentProcessor(resourceAssignments, ctx);
  ...
  private void processBatchResourceAssignments(ResourceAssignmentData resourceAssignmentData, SvcLogicContext ctx,
      Map<String, Object> componentContext, List<ResourceAssignment> batchResourceAssignment) throws SvcLogicException {
    ...
    if (CollectionUtils.isNotEmpty(batchResourceAssignment)) {
      ...
      if (batchFirstResourceAssignment != null
        ...
        inParams.put(ConfigAssignmentConstants.INPUT_PARAM_RESOURCE_TYPE, resourceAssignmentData.getResourceType());
