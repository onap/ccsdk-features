      ...
      if (capabilityAssignment != null && capabilityAssignment.getProperties() != null) {
        ...
        if (mappingObject != null) {
          ...
          if (StringUtils.isNotBlank(mappingContent)) {
            this.assignments = TransformationUtils.getListfromJson(mappingContent, ResourceAssignment.class);
          } else {
            validationMessage.append(String.format("Failed to transform Mapping Content (%s) ", mappingContent));
            throw new ConfigModelException(String.format("Failed to transform Mapping Content (%s) ", mappingContent));
          }
        }
      }
    }
    ...
    this.assignments.forEach(resourceMapping -> {
      if (resourceMapping != null) {
        if (!resourceAssignmentMap.containsKey(resourceMapping.getName())) {
          ...
          validationMessage
              .append(String.format("Duplicate Assignment Template Key (%s) is Present", resourceMapping.getName()));
        }
      }
    });
    ...
    if (!assignments.isEmpty()) {
      ...
      for (ResourceAssignment resourceAssignment : assignments) {
        if (resourceAssignment != null) {
          ...
          if (!added) {
            validationMessage
                .append(String.format("Duplicate Assignment Dictionary Key (%s) present with Template Key (%s)",
