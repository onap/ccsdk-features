          ...
          if (StringUtils.isNotBlank(nodeTemplateContent)) {
            ...
            if (nodeTemplate != null && StringUtils.isNotBlank(nodeTemplate.getType())) {
              String nodeTypeContent =
                  context.getAttribute(ConfigModelConstant.PROPERTY_NODE_TYPES_DOT + nodeTemplate.getType());
              NodeType nodetype = TransformationUtils.readValue(nodeTypeContent, NodeType.class);
              if (nodetype != null) {
                inParams.put(ConfigModelConstant.PROPERTY_CURRENT_NODETYPE_DERIVED_FROM, nodetype.getDerivedFrom());
                ...
                throw new SvcLogicException(String.format("Failed to get node type (%s) for node template (%s).",
                    nodeTemplate.getType(), componentKey));
              }

            } else {
              throw new SvcLogicException(
                  String.format("Failed to convert content (%s) to node template.", nodeTemplateContent));
            }
          } else {
            throw new SvcLogicException(
                String.format("Couldn't get node template content for component key (%s).", componentKey));
          }
        } else {
          throw new SvcLogicException(String.format("Couldn't get component key (prefix) from inparam (%s)", inParams));
        }
      }
    } catch (Exception e) {
      throw new SvcLogicException(e.getMessage());
  ...
  public String getNodeTemplateContent(final SvcLogicContext context, String templateName) throws SvcLogicException {
    ...
    try {
      if (context != null && StringUtils.isNotBlank(templateName)) {
        ...
        content = context.getAttribute(ConfigModelConstant.PROPERTY_NODE_TEMPLATES_DOT + templateName + ".content");
      }
    } catch (Exception e) {
      throw new SvcLogicException(e.getMessage());
    }
        ...
        if (StringUtils.isNotBlank(templateName)) {
          ...
          if (StringUtils.isNotBlank(nodeTemplateContent)) {
            ...
            if (nodeTemplate != null && nodeTemplate.getCapabilities() != null
                && nodeTemplate.getCapabilities().containsKey(ConfigModelConstant.CAPABILITY_PROPERTY_MAPPING)) {
              ...
              if (capability.getProperties() != null) {
                List mappingList =
                    (List) capability.getProperties().get(ConfigModelConstant.CAPABILITY_PROPERTY_MAPPING);
                if (mappingList != null) {
