    ...
    if (node != null && StringUtils.isNotBlank(node.asText())) {
      ...
      if (expressionMap != null) {
        ...
        if (expressionMap.containsKey(ConfigModelConstant.EXPRESSION_GET_INPUT)) {
          ...
          isExpressionNode = true;
          expressionValue = context.getAttribute(expressionMap.get(ConfigModelConstant.EXPRESSION_GET_ATTRIBUTE));
        ...
        logger.debug("expression ({}), expression key ({}), expression Value ({})", expressionMap, expressionKey,
            expressionValue);
      }
    }
  }
